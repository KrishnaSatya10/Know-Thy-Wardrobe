...
class DataPreparation:
    ...
    def __init__(
        ...
        if transform:
            self.transform = transforms.Compose(
                [transforms.Resize(self.config.image_size), transforms.ToTensor()]
            )
        if target_transform:
    ...
    """
        return DataLoader(
            self.training_data, batch_size=self.config.batch_size, shuffle=True
        )

    def test_dataloader(self):
        return DataLoader(
            self.test_data, batch_size=self.config.batch_size, shuffle=True
        )
